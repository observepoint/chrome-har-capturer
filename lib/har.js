...
function parseEntry(pageref, entry) {
    ...
    combineExtraHeaders(headers, entry);
    // parse cookie data from extra headers
    const responseCookies = gatherCookies(entry);
...
function combineExtraHeaders(currentHeaders, entry) {

}

function gatherCookies(entry) {
    if (entry.responseExtraParams && entry.responseExtraParams.headers['set-cookie']) {
        const headerCookies = entry.responseExtraParams.headers['set-cookie'].split('\n');
        return headerCookies.map(cookie => createCookieFromString(cookie));
    }
    return [];
}

function createCookieFromString(cookieString) {
    return cookieString
        .split(';')
        .map(cookiePart => cookiePart.split('='))
        .reduce((acc, cookiePartKV, index) => {
            const key = decodeURIComponent(cookiePartKV[0].trim());
            if (index === 0) {
                acc['name'] = key;
                acc['value'] = decodeURIComponent(cookiePartKV[1].trim());
            }

            // single word cookie-key cases can be cascaded. Multi-word cookie keys are explicitly set to prevent
            // other casing scenarios from servers being used and breaking the expected cookie object shape.
            ...
            case 'size':
            case 'comment':
                acc[key.toLowerCase()] = decodeURIComponent(cookiePartKV[1].trim());
                break;
            case 'sourcescheme':
                acc['sourceScheme'] = decodeURIComponent(cookiePartKV[1].trim());
                break;
            case 'samesite':
                acc['sameSite'] = decodeURIComponent(cookiePartKV[1].trim());
                break;
            case 'sourceport':
                acc['sourcePort'] = decodeURIComponent(cookiePartKV[1].trim());
                break;
            case 'partitionkey':
                acc['partitionKey'] = decodeURIComponent(cookiePartKV[1].trim());
                break;
            case 'secure':
            case 'session':
