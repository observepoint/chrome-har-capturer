...
class Stats {
    ...
    _Network_requestWillBeSentExtraInfo(fulfill, reject, params) {
        ...
        Object.keys(newHeaders).forEach(newHeader => {
            const alreadyPresentExact = Object.keys(entry.requestParams.request.headers)
                .filter(existingHeader => existingHeader.toLowerCase() === newHeader.toLowerCase())
                .some(existingHeader => entry.requestParams.request.headers[existingHeader] === newHeaders[newHeader]);
